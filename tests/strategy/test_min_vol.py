...
def test_min_vol_signal_initialization(universe_spec, mock_price_data, mock_benchmark_data):
    ...
    with patch('src.data_access.prices.PriceDataFetcher.get_price_data') as mock_get_price_data, \
         patch('src.strategy.min_vol.min_vol.MinVolSignalData', return_value=mock_min_vol_data):
...
def test_calculate_signal_scores(universe_spec, mock_price_data, mock_benchmark_data):
    ...
    with patch('src.data_access.prices.PriceDataFetcher.get_price_data') as mock_get_price_data, \
        ...
        assert min_vol_scores.select_dtypes(include=[np.number]).shape[1] == len(expected_columns)
