...
def test_construct_portfolio_weights_basic():
    # Create sample data
    data = pd.DataFrame(
        {"AAPL": [1.0, 2.0, 3.0], "MSFT": [2.0, 3.0, 4.0], "GOOGL": [3.0, 4.0, 5.0]},
...
def test_construct_portfolio_weights_zero_data():
    ...
    data = pd.DataFrame(
        {"AAPL": [0.0, 0.0, 0.0], "MSFT": [0.0, 0.0, 0.0], "GOOGL": [0.0, 0.0, 0.0]},
        index=pd.date_range("2023-01-01", periods=3),
    )
...
def test_construct_portfolio_weights_single_asset():
    ...
    data = pd.DataFrame(
        {"AAPL": [1.0, 2.0, 3.0]}, index=pd.date_range("2023-01-01", periods=3)
    )
