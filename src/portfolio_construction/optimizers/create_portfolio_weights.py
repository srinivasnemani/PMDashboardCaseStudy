...
def construct_portfolio_weights(signal_df: pd.DataFrame, strategy_name: str, top_n: int = 25) -> pd.DataFrame:
    """
    ...
    for date, row in signal_df.iterrows():
        ...
        n_assets: int = min(top_n, len(sorted_scores) // 2)  # Ensure balanced long/short
