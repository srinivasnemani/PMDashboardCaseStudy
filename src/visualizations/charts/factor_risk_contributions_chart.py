...
def plot_risk_contribution_by_factor(df):
    ...
    df_nonzero_risk = df[
        (df["Risk Contribution"] != 0)
        & (~df["Factor"].isin(["Total", "Specific Risk"]))
    ]
    ...
    fig = make_subplots(
        ...
        subplot_titles=(
            "Factor Risk Decomposition (% of Gross Absolute Contributions)",
            "Active Factor Tilts",
        ),
    )
    ...
    factor_colors = {
        factor: color_palette[i % len(color_palette)]
