...
def render_exposure_time_series(
    df: pd.DataFrame, measure_type: Literal["USD", "Pct", "PnL"] = "USD"
) -> go.Figure:
    """
    ...
    if measure_type == "USD":
        ...
        fig.add_trace(
            go.Scatter(
                x=df["trade_open_date"], y=df["long_pnl"], mode="lines", name="Long PnL"
            )
        )
        fig.add_trace(
            go.Scatter(
...
def render_leverage_time_series(
    df: pd.DataFrame,
    measure_type: Literal["Leverage", "Capital", "Target Exposure"] = "Leverage",
) -> go.Figure:
    """
    ...
    if measure_type == "Leverage":
        ...
        fig.add_trace(
            go.Scatter(x=df["date"], y=df["aum"], mode="lines", name="Capital")
        )
        ...
        fig.add_trace(
            go.Scatter(
                x=df["date"],
                y=df["calculated_target_exposure"],
                mode="lines",
