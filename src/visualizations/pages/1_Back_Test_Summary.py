...
def display_trade_summary(trades_summary, title):
    ...
    if 'Metric' in trades_summary.columns and 'Value' in trades_summary.columns:
        trades_summary['Value'] = trades_summary.apply(
            lambda row: (
                f"{row['Value'] * 100:.2f}%" if row['Metric'] in percentage_based_measures and pd.notnull(row['Value'])
                else f"{row['Value']:,.2f}" if row['Metric'] in decimal_based_measures and pd.notnull(row['Value'])
                else row['Value']
            ),
            axis=1
        )
    ...
    st.download_button("Download Table as CSV", csv, "trades_summary.csv", "text/csv", key=f"download_{title}")
