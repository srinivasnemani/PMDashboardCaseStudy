...
# This page provides detailed performance analysis across different dimensions:
# - P&L analysis by trade direction (long/short)
# - P&L breakdown by GICS sectors
# - Interactive time series visualizations
# - Detailed tabular data with downloadable reports
# =============================================================================

st.set_page_config(layout="wide")
st.markdown(
    "<h2 style='text-align: center;'>Performance Analysis</h2>", unsafe_allow_html=True
)
...
def render_pnl_ts_chart_by_trade_direction(exposure_ts_df):
    ...
    with exposures_tab_2:
        ...
        st.download_button(
            ...
            csv,
            "pnl_time_series_by_trade_direction.csv",
            "text/csv",
        )
        ...
        dollar_cols = [
            ...
            if any(key in col for key in ["exposure", "pnl"])
            and not col.endswith("pct")
        ]
        percent_cols = [
            col
            for col in exposure_ts_df.columns
            if col.endswith("pct") or col.startswith("cumulative_")
        ]
        for col in exposure_ts_df.columns:
...
def render_pnl_time_series_gics_sectors(exposure_ts_df):
    ...
    with exposures_tab_2:
        ...
        st.download_button(
            "Download Table as CSV",
            csv,
            "pnl_time_series_by_gics_sector.csv",
            "text/csv",
        )
        gb = GridOptionsBuilder.from_dataframe(exposure_ts_df)
        gb.configure_column(
...
def render_performance_page():
    ...
    st.markdown(
        f"<h6 style='text-align: left;'>Analysis for the strategy: {strategy_name}</h6>",
    ...
    pnl_group_by_trade_direction = fetch_pnl_exposures_ts(
