...
def render_risk_attribution_page():
    ...
    st.subheader(f"Analysis for the strategy: '{strategy_name}' and for the rebalance date: '{selected_date}'")
...
def render_trade_summary_by_direction(exposures_summary_by_direction):
    ...
    with tab_2:
        ...
        st.download_button("Download Table as CSV", csv, "exposures_by_direction.csv", "text/csv")
        gb = GridOptionsBuilder.from_dataframe(exposures_summary_by_direction)
        for col in exposures_summary_by_direction.columns:
            if col in ["exposure", "Long", "Short", "gross_exposure_usd", "net_exposure_usd"]:
...
def render_trade_summary_by_total_net(exposures_summary_by_direction):
    ...
    with tab_2:
        csv = exposures_summary_by_direction.to_csv(index=False)
        st.download_button("Download Table as CSV", csv, "exposures_by_total_net.csv", "text/csv")
        gb = GridOptionsBuilder.from_dataframe(exposures_summary_by_direction)
        for col in exposures_summary_by_direction.columns:
            if col in ["exposure", "Long", "Short", "gross_exposure_usd", "net_exposure_usd"]:
                gb.configure_column(col, type=["numericColumn"],
                ...
                gb.configure_column(col, type=["textColumn"])
        gridOptions = gb.build()
        AgGrid(exposures_summary_by_direction, gridOptions=gridOptions, fit_columns_on_grid_load=True, theme="compact")
...
def render_trade_data_analysis_page():
    ...
    filtered = exposures_summary_by_direction[~exposures_summary_by_direction.index.isin(['Total', 'Specific'])]
    assert math.isclose(filtered["exposure_pct"].sum(), 100.0, rel_tol=1e-4, abs_tol=1e-4)
