...
def render_exposure_ts(exposure_ts_df):
    ...
    with tab_2:
        ...
        st.download_button("Download Table as CSV", csv, "exposures_time_series.csv", "text/csv")
        ...
        dollar_cols = [
            "long_exposure", "short_exposure", "long_pnl", "short_pnl", "total_exposure", "net_exposure", "total_pnl",
            "cumulative_long_pnl", "cumulative_short_pnl", "cumulative_total_pnl"
        ]
        percent_cols = [
            "long_pnl_pct", "short_pnl_pct", "net_pnl_pct", "cumulative_long_pnl_pct", "cumulative_short_pnl_pct", "cumulative_total_pnl_pct"
        ]
...
def render_capital_and_leverage_ts(aum_leverage_df):
    ...
    with leverage_tab_4:
        ...
        if "aum" in aum_leverage_df.columns:
            gb.configure_column("aum", type=["numericColumn"], valueFormatter="x == null ? '' : x.toLocaleString(undefined, {minimumFractionDigits: 0, maximumFractionDigits: 0})")
        if "calculated_target_exposure" in aum_leverage_df.columns:
            gb.configure_column("calculated_target_exposure", type=["numericColumn"], valueFormatter="x == null ? '' : x.toLocaleString(undefined, {minimumFractionDigits: 0, maximumFractionDigits: 0})")
        if "target_leverage" in aum_leverage_df.columns:
            gb.configure_column("target_leverage", type=["numericColumn"], valueFormatter="x == null ? '' : x.toFixed(2)")
        gridOptions = gb.build()
        AgGrid(aum_leverage_df, gridOptions=gridOptions, fit_columns_on_grid_load=True, theme="compact")
