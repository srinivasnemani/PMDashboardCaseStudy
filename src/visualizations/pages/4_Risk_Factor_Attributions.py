...
def render_pnl_attributions(pnl_attribution_df):
    ...
    with tab_2:
        ...
        st.download_button("Download Table as CSV", csv, "pnl_attribution.csv", "text/csv")
        gb = GridOptionsBuilder.from_dataframe(pnl_attribution_df)
        gb.configure_column("sector", type=["textColumn"])
        gb.configure_column("factor_pnl_usd", type=["numericColumn"],
                            valueFormatter="x == null ? '' : x.toLocaleString(undefined, {minimumFractionDigits: 0, maximumFractionDigits: 0})")
        gb.configure_column("factor_exposure_pct", type=["numericColumn"],
                            valueFormatter="x == null ? '' : (x * 100).toFixed(2) + '%'")
        gb.configure_column("pnl_contribution_pct", type=["numericColumn"],
                            valueFormatter="x == null ? '' : (x * 100).toFixed(2) + '%'")
        gridOptions = gb.build()
        AgGrid(pnl_attribution_df, gridOptions=gridOptions, fit_columns_on_grid_load=True, theme="compact")
    ...
    with tab_2:
        ...
        for col in risk_decomp_df.columns:
            if col.endswith('(Annualized Variance)'):
                gb.configure_column(col, type=["numericColumn"], valueFormatter="x == null ? '' : x.toFixed(4)")
                ...
                gb.configure_column(col, type=["numericColumn"], valueFormatter="x == null ? '' : x.toFixed(2)")
            else:
                gb.configure_column(col, type=["numericColumn"], valueFormatter="x == null ? '' : x.toFixed(2)")
        gridOptions = gb.build()
        AgGrid(risk_decomp_df, gridOptions=gridOptions, fit_columns_on_grid_load=True, theme="compact")
    ...
    with tab_2:
        ...
        for col in factors_risk_contribution_df.columns:
            if col in ["factor_pnl_usd"]:
                ...
                gb.configure_column(col, type=["numericColumn"], valueFormatter="x == null ? '' : x.toFixed(2)")
        gridOptions = gb.build()
        AgGrid(factors_risk_contribution_df, gridOptions=gridOptions, fit_columns_on_grid_load=True, theme="compact")
